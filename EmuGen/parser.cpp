...
int main(int argc, char **argv)
{
   ...
   std::string path = "D:\\Programming\\Projects\\xbox360-emu\\EmuGen\\powerpc.txt";
   std::fstream in;
   in.open(path, std::ifstream::in);
   ...
   for (std::string line; std::getline(in, line); ) {
      ...
      fileContents.append(line.begin(), line.end());
   ...
   if (parser_root.parse<decltype(first), decltype(ctx), false>(first, last, ctx, result) && first == last) {
      return 1;
   } else {
      auto errorPos = first;
      auto line = getLine(fileContents.begin(), errorPos, fileContents.end());
      std::cout << "Syntax Error:" << std::endl;

      std::cout << std::string(line.first, line.second) << std::endl;

      for (auto itr = line.first; itr != errorPos; ++itr) {
         std::cout << ' ';
      }
