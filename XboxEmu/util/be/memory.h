...
public:
   ...
   template<typename T>
   static inline void write(uint64_t address, T &value)
   {
      *reinterpret_cast<T*>(address) = value;
   }

   template<>
   static inline void write(uint64_t address, uint16_t &value)
   {
      *reinterpret_cast<uint16_t*>(address) = bits::swap(value);
   }

   template<>
   static inline void write(uint64_t address, uint32_t &value)
   {
      *reinterpret_cast<uint32_t*>(address) = bits::swap(value);
   }

   template<>
   static inline void write(uint64_t address, uint64_t &value)
   {
